...
@bot.event
async def on_ready():
	activity = discord.Game(name="v!sms [เบอร์] [จำนวน1-200]")
...
async def sms(ctx, no=None, jam:int=None):
	try:
		if (str(ctx.message.channel.id) == "1042408516494823535"):
			if (no == None or jam == None):
				...
				phone = no
				jam = jam
				embed = discord.Embed(title="✅ SUCCESS",description=f"ATTACKING TO [PHONE] > ||{no}|| [AMOUNT] > {jam}/200 ",color=0x45fc03)
				embed.set_image(url="https://media.discordapp.net/attachments/1003098176091336784/1053209021479391272/standard_2.gif")
...
bot.run("MTA0NzExNTExMzQ1MjI4NjAzMg.G1egpE.3eOwOHqM1RiECN9LA52JqAY6yUpGfBFnrelWDc")
